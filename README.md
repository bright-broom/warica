...
2026-09-14のローカル検証では、単体**35件成功**、ブラウザ操作**125件成功・対象外1件**を確認しました。ブラウザはChromiumのPC（1440×1100）・スマホ（320×800）、WebKitのスマホ（390×844）の3構成です。対象外の1件はスマホ専用レイアウト検査のPC実行です。追加で縦横表示・タブレット・PCの7サイズも確認しています。テスト件数はこの時点の記録です。
