...
既定はポップなミントのデザインです。丸いカード、濃い縁取り、硬い影、ドット背景、Lucideのステッカー風装飾で親しみやすさを表現します。主要操作は右側に維持し、装飾には操作を割り当てません。新しい基本色・外部フォント・画像依存は追加しません。

右上メニューの「以前のデザインに戻す」で従来の見た目へ即座に切り替えられます。「ポップなデザインにする」で再選択できます。設定は `warica-appearance-v1` に保存し、再読み込み前の初期描画にも適用します。イベント・支払い・下書きとは別の設定で、切り替えによる画面の作り直しやデータ消去はありません。設定の保存に失敗しても、その画面では切り替えを維持して通知します。

`src/design/appearance.ts` が設定キーと初期描画スクリプト、`AppearanceProvider` が状態と切り替えを管理します。見た目は `globals.css` の `[data-appearance='pop']` に限定し、共通部品の `data-ui` / shadcnの `data-slot` を参照します。ページごとにテーマ分岐を持たせません。ポップ専用の装飾は `.pop-only` で従来デザインから隠します。

従来の角丸はカード16px・操作10px、ポップはカード24px・操作16pxです。フォントと角丸は `--app-*`、ポップ固有の派生色は `--pop-*` で管理します。ホバーは動きを減らす設定で無効になり、タッチ端末では発生しません。

コードの復元地点はGitタグ `design-before-pop-2026-09-13`（`9720f4d`）です。今後の変更を残しながら今回の変更だけ戻す場合は、デザイン刷新のコミットをrevertしてください。画面の切り替えだけならGit操作は不要です。
...
操作は原則アイコンのみとし、`label` からアクセシブル名とツールチップを渡します。入力ラベル・ページ見出しは読み上げ向けにも保持します。セクションには短い見出しを表示し、操作部品は原則48px・最低44pxを確保します。フォーカスは通常面でメイン色、濃いナビ上でアクセント色の輪郭を使います。角丸は上記のテーマトークンで統一します。
