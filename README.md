# WARICA

**楽しい時間の、そのあとに。** 旅行や食事の立て替えを記録し、誰が誰にいくら送ればよいかを計算する、日本語の割り勘アプリです。

## できること

- 初回はイベント「みんなでごはん」・メンバー「A」「B」で開始。名前の入力なしで支払いを記録できます。保存済みのイベントは変更しません。
- イベント名とメンバーを登録・編集（2〜100人）
- 支払いを追加・編集・削除（1件1〜1,000,000円、整数の日本円のみ）
- 支払い履歴は新しい順に20件ずつ表示。内容・支払者・対象者の名前で全件検索でき、全角・半角を揃えて検索します。空白で区切るとすべての語を含む記録に絞ります。検索やページ送りで合計・精算・一括コピーの対象は変わりません。
- 全角数字・桁区切りの金額貼り付け、内容アイコンからの入力、同じ支払者・対象者での連続追加
- 未登録の支払い・編集中の入力を、同じタブ内の画面移動・再読み込みで保持（タブを閉じると下書きは終了）
- 入力開始時の支払者・対象者を保持し、後から追加したメンバーを自動では含めません。選択した人を削除した場合は、支払者の選び直し・残った対象者の確認が済むまで登録を止め、金額と内容を残します。
- 支払いごとに割り勘の対象者を選択。支払った本人を含める・外す、どちらも可能
- 支払額、負担額、精算額と送金先を表示
- 精算画面の「一括コピー」でイベント名・合計・全員分の送金者／送金先／金額をコピーし、LINEに貼り付け。相手別コピーと、コピーできない場合の選択可能なテキスト表示にも対応
- 精算ごとにPayPayの請求リンクを登録・編集・削除。PayPayを開くボタンと、リンク付きの相手別コピー
- ブラウザ内の自動保存、右上のリフレッシュアイコンでデータと支払いの下書きを保持してリフレッシュ。保存トラブル時はJSONバックアップで復旧（2MBまで）
- PC・スマートフォンに対応。スマホのボタン・対象者チェックを右側に集約し、画面移動はページ内の矢印に統一。本文は画面下まで使用し、ホームインジケーター周辺には余白を確保。320px・390px幅で操作位置、44px以上のタップ領域、横スクロールがないことを自動テスト
- ポップなミントのデザインに統一。右上には循環する矢印のリフレッシュアイコンだけを直接表示し、デザイン切り替え・イベント全消去はありません。

実際の送金、共同編集、ログイン、クラウド同期は行いません。入力したデータを送るAPIや外部フォントの読み込みはありません。PayPayリンク・公式ガイドを開く操作では外部サイトへ遷移します。サイトにアクセスした際の通信・ホスティング側のアクセス記録は別です。

## PayPayで送る

1. 受け取る人が、精算額に対応する請求リンクをPayPayで作成します。[公式の手順](https://paypay.ne.jp/guide/receive/)に従い、LINEの「送る・受け取る」から金額を指定して共有できます。請求リンクの作成にはPayPayの本人確認が必要です。
2. WARICAの精算欄で「PayPay」を押し、発行済みのリンクを貼り付けて登録します。送金する人は、登録後の「PayPay」またはコピーした相手別の送金内容からリンクを開きます。
3. PayPay側で宛先・金額を確認して送金します。リンクの種類・有効期限・宛先・金額・送金完了をWARICAが照会する機能はありません。リンクを開いても精算済みにはしません。

公開APIで個人間送金を作成する連携ではなく、発行済みリンクをそのまま開く方式です。URLに金額や宛先を追加・変更しません。入力時・保存データ読み込み時にHTTPSとPayPayのドメインを検査しますが、これだけで正しい請求内容を保証するものではありません。実際のPayPayアプリへの遷移は端末・ブラウザ・アプリ設定に依存します。

リンクはイベントの保存・復旧用JSONに含まれます。精算の送金者・受取者・氏名・金額が変わったリンクは解除し、金額を戻しても復活させません。別タブによる更新や保存失敗を検知した場合は遷移を停止して入力を保持します。旧データの対象者確認が終わるまでは登録できません。

ブラウザテストは架空のリンクへの通信を遮断・置換して、URLの受け渡しとWARICA内の動作を検証します。実際のPayPayアプリ起動・送金は検証に含みません。

## 起動

Node.js 24を推奨（22以上）、pnpm 10.23.0を使用します。

```sh
git clone git@github.com:bright-broom/warica.git
cd warica
corepack enable
pnpm install --frozen-lockfile
pnpm dev
```

[localhost:3000](http://localhost:3000) を開いてください。環境変数やデータベースは不要です。

```sh
pnpm lint       # ESLint（警告もエラーにする）
pnpm typecheck  # Next.jsのルート型生成とTypeScriptチェック
pnpm test       # 計算・バリデーション・保存の単体テスト
pnpm build      # 本番向けビルド
pnpm check      # 静的検査・未使用コード・単体テスト・ビルド
pnpm start      # ビルド済みのアプリを起動

pnpm exec playwright install chromium webkit
pnpm test:e2e   # 本番ビルドでPC・320px幅・モバイルWebKitの操作とアクセシビリティを検査（先にpnpm build）

# 公開環境を確認。ローカルサーバーは起動しません
PLAYWRIGHT_BASE_URL=https://warica.vercel.app pnpm test:e2e --workers=2
```

GitHub ActionsはNode.js 24で静的チェック、単体テスト、ビルド、依存関係監査、ブラウザテストを実行します。ブラウザテストはChromium（PC・320px幅）とWebKit（390px幅・タッチ操作）の3構成です。WebKitだけを実行する場合は `pnpm test:e2e --project=mobile-webkit` を使用します。実機のSafari検証とは区別しています。

公開環境の確認結果と再実行方法は[検証記録](docs/PRODUCTION_VERIFICATION.md)を参照してください。

大量データの保存処理は `pnpm benchmark:storage` で計測できます。[計測条件と改善結果](docs/STORAGE_PERFORMANCE.md)を参照してください。

## 計算ルール

1. 1回の立て替えを1件の支払いとして保存します。対象者は名前ではなくIDで保持します。
2. 各支払いの金額を、その支払いの対象者だけに均等配分します。
3. 1円未満の端数は対象者の登録順に1円ずつ割り当てます。例：1,000円を3人で分けると334円・333円・333円です。
4. 各人の「支払額 − 負担額」を求め、受け取る人と支払う人の間で差額を相殺します。
5. 送金は最大で人数−1件にまとめます。貪欲法のため、数学的に最小の送金件数を常に保証するものではありません。

例：A・B・Cのうち、AがBだけのために3,000円を立て替えた場合、BからAへ3,000円、Cの負担は0円です。AとBの2人を対象にすると、BからAへ1,500円です。

新しくメンバーを追加しても、登録済みの支払いの対象者は変わりません。支払いに含まれるメンバーは、その支払いを先に編集・削除するまで削除できません。

## 保存と復旧

- 1つのProviderが全画面で状態を共有し、更新時に同期的にlocalStorageへ保存します。画面移動時に500msの保存待ちをキャンセルしていた旧実装を廃止しました。
- 支払いの下書きはタブごとのsessionStorageへ同期保存します。新規入力と編集中の入力を別々に保持し、再読み込み後も復元します。登録済みデータの更新日時と照合し、別イベントの入力は復元しません。バックアップ読み込みで下書きをクリアします。下書きはJSONバックアップに含まれません。
- 旧形式の入力途中の下書きで、既定の支払者・対象者が省略されている場合は、読み込んだイベントのメンバーで選択を確定します。明示的な選択や削除されたIDは保持し、勝手に別の人へ置き換えません。未入力の下書きは、入力を始めた時点のメンバーを既定値にします。
- 保存失敗時は入力をメモリに残して警告と復旧操作を表示します。右上のリフレッシュアイコンは登録データと下書きの保存成功後だけ実行します。ブラウザから再読み込み・終了する場合も、未保存なら離脱確認を要求します。ただしOSによる強制終了など、ブラウザが離脱確認を実行しない場合の保持は保証できません。
- 通常の画面下には管理操作を置きません。「バックアップ」「読み込む」は保存トラブル時の復旧パネルに表示します。
- 保存前に読んだ内容と現在の保存内容を比較し、別タブが更新していたら上書きを止めます。別タブの保存や画面へのフォーカス復帰でも変更を検知して通知し、現在の入力は保持します。競合時は「最新の保存データを読み込む」または右上のリフレッシュアイコンから、未保存変更・下書きの破棄を確認して最新の保存内容へ切り替えられます。必要なら先に現在のイベントをバックアップしてください。読み取りや下書き消去に失敗した場合は切り替えず、入力を保持します。最新データの読み込みはlocalStorageへ書き戻しません。複数タブの変更を自動統合する機能はありません。
- 前回の正常な保存データをバックアップキーに保持します。主データが壊れていたらそこから読み込み、復旧したことを表示します。バックアップの書き込みだけが失敗した場合は、主データの保存を試みます。
- バックアップの読み込みは、保存できた場合にだけ現在のイベントと下書きを置き換えます。容量不足や別タブとの競合では現在の入力を維持し、読み込みを成功扱いにしません。復旧時も更新日時を切り替え、下書きの消去に失敗した場合に古い入力を再読み込みで復元しないようにします。
- 両方を読み込めない場合、元データを消さずに編集を止めます。ブラウザのアクセス設定を確認するか、「読み込む」から保存済みのJSONバックアップを選び、置き換えを確認してください。
- JSONファイルは暗号化されません。ブラウザのサイトデータを削除したり、別の端末・URL・ブラウザを使ったりした場合は、バックアップを読み込んでください。

### 旧バージョンのデータ

既存キー `warican-app-data-v2` / `warican-backup-v2` を引き継ぎ、2.0.0・3.0.0形式を読み込めます。請求リンクを含む保存形式は3.1.0です。

旧バージョンは選択した対象者を保存せず、支払いを分割してメモに名前を埋め込んでいました。そのため、正確な対象者の自動復元はできません。旧レコードには読み込み時の全員を仮設定し、「旧データ・対象者を要確認」を表示します。支払いを編集して対象者を確認・保存すると表示が消えます。

メモから対象者を推測したり、複数レコードを自動でまとめたりはしません。元の記録は個別のまま残ります。端数の配分も支払い単位になるため、旧画面の結果と差が出る場合があります。旧記録を確認してから精算してください。小数金額・不明なメンバー参照などの不正な記録は黙って修正せず、読み込みを停止します。

## 構成

- `src/app/`：メンバー・支払い・精算結果の3画面と共有状態
- `src/design/theme.ts`：全画面・faviconの3色パレット
- `src/config/navigation.ts`：ページ情報と表示条件
- `src/components/ui/`：shadcn/ui（Radix）の基礎部品とWARICAの共通プリセット
- `src/components/ApplicationUI.tsx`：Tooltip・確認ダイアログ・Sonnerの共通管理
- `src/components/`：最上位の画面枠、アイコン操作、支払いフォーム、支払い履歴
- `src/components/PaymentWorkspace.tsx`：画面移動・再読み込みで維持する入力下書き（登録済みデータと分離）
- `src/lib/calculations.ts`：円単位の負担配分・送金計算・共有テキスト
- `src/lib/storage.ts`：保存、バックアップ、旧形式読み込み、インポート検証
- `src/lib/validation.ts`：入力の制限と対象者の検証
- `tests/`：実例と100パターンの台帳に対する金額保全・精算検証、保存異常、ブラウザ操作

Next.js 15.5、React 19、TypeScript 5.9、Tailwind CSS 4、shadcn/ui（Radix）、Lucideを使用しています。Next.jsは[公式の2026年8月セキュリティ更新](https://nextjs.org/blog/august-2026-security-release)より新しい15.5.25へ更新し、Next.jsが固定するPostCSSは互換性のある8.5.28へ上書きしています。公開前にも依存関係監査を実行してください。

色・部品・ページ構成の変更方法は[デザイン管理ガイド](docs/DESIGN_SYSTEM.md)を参照してください。
