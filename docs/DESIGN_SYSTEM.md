...
- `main`: 深いティール。文字・線・精算結果・共通ナビの背景。
- `sub`: 淡いオフホワイト。カード・入力欄の背景。ページ地は `theme.ts` の `--brand-canvas` でメイン色4%とサブ色を混ぜ、カードとの明度差を作ります。
...
グラデーションの色・方向・濃度は `theme.ts` の共通変数で管理します。アクセントから色相を24度グリーン側に寄せ、サブ色と混ぜた淡いミントから青緑へ135度の方向につなげています。[CSSの相対色](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Colors/Using_relative_colors)を使用し、非対応環境では単色の背景が残ります。本文の文字色は維持し、濃いナビ上ではサブ色、現在位置のミント面ではメイン色のアイコンを使います。
...
操作は原則アイコンのみとし、`label` からアクセシブル名とツールチップを渡します。入力ラベル・ページ見出しは読み上げ向けにも保持します。セクションには短い見出しを表示し、操作部品は原則48px・最低44pxを確保します。フォーカスは通常面でメイン色、濃いナビ上でアクセント色の輪郭を使います。カードは16px、入力・操作部品は10pxの角丸で統一します。

## 確認

`pnpm check:design` はテーマ外の色コード、標準色パレット、ページでのレイアウト重複・直接の入力部品を検出します。`pnpm lint` とCIからも実行されます。

ブラウザテストでは、ChromiumのPC・320px幅とWebKitの390px幅で登録・編集・精算・保存失敗・復旧を確認します。下部ナビゲーションとの重なりは、プロジェクト名ではなく `isMobile` 設定に基づいて全モバイル構成で検査します。axeによるラベル・コントラスト等の検査は、各画面・入力前後・支払い履歴・確認画面で実施します。通知やダイアログの表示アニメーションが終わってから評価し、検査ルールや対象要素の除外は行いません。CSS変数の変更が3画面の主要操作に反映され、画面遷移で共通レイアウトが作り直されないことも検証します。

参考: [Tailwind CSSのテーマ変数](https://tailwindcss.com/docs/theme)

## 入力と移動

スマートフォンでは操作ボタン・対象者チェックを右側に集めます。候補アイコンは狭い画面で2列、ヘルプ・コピー・確認ダイアログも右寄せにします。共通ナビは右下に固定し、セーフエリア込みの専用領域を確保します。本文は残りの高さの中でスクロールするため、ナビに入力欄や操作が隠れません。ルート移動時は本文のスクロールを先頭へ戻します。スマホのロゴは表示のみとし、ホームへは右下ナビから移動します。PCでは通常のページスクロールと左側ナビを維持します。

`PaymentWorkspaceProvider` は未登録の支払いと編集中の支払いを別々に保持します。ページを往復しても入力は残り、編集キャンセルでは元の未登録入力へ戻ります。新規追加後は支払者・対象者を残し、金額へフォーカスを戻します。下書きはsessionStorageに同期保存して同じタブ内の再読み込みでも復元し、イベントのリセット・インポートでは破棄します。下書きはバックアップや精算計算へ含めません。

競合通知の復帰ボタンも右側に配置します。競合は保存結果のエラーコードと別タブの変更通知で識別し、文言の一致で分岐しません。確認ダイアログを閉じた後、Storeが最新の保存内容を検証し、Workspaceが下書きを消去できた場合だけStoreが表示データを切り替えます。イベントを明示的に置き換えたときは `eventRevision` を使ってページ内の未確定入力もクリアし、本文を先頭へ戻します。AppShellとProviderは維持します。メニューから確認画面を開く場合は、メニューを閉じてフォーカスを戻してから開きます。
