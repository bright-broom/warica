...
- ポップなミントのデザイン。右上メニューの「以前のデザインに戻す」で従来の見た目へ戻せます。入力・支払いはそのまま、選んだデザインは再読み込み後も保持します。
...
リンクはイベントの保存・復旧用JSONに含まれます。精算の送金者・受取者・氏名・金額が変わったリンクは解除し、金額を戻しても復活させません。新しいイベントにも引き継ぎません。別タブによる更新や保存失敗を検知した場合は遷移を停止して入力を保持します。旧データの対象者確認が終わるまでは登録できません。
...
- 保存前に読んだ内容と現在の保存内容を比較し、別タブが更新していたら上書きを止めます。別タブの保存や画面へのフォーカス復帰でも変更を検知して通知し、現在の入力は保持します。競合時は「最新の保存データを読み込む」またはメニューの「リフレッシュ」から、未保存変更・下書きの破棄を確認して最新の保存内容へ切り替えられます。必要なら先に現在のイベントをバックアップしてください。読み取りや下書き消去に失敗した場合は切り替えず、入力を保持します。最新データの読み込みはlocalStorageへ書き戻しません。複数タブの変更を自動統合する機能はありません。
- 前回の正常な保存データをバックアップキーに保持します。主データが壊れていたらそこから読み込み、復旧したことを表示します。バックアップの書き込みだけが失敗した場合は、主データの保存を試みます。
- バックアップの読み込み・新しいイベントの開始は、保存できた場合にだけ現在のイベントと下書きを置き換えます。容量不足や別タブとの競合では現在の入力を維持し、読み込みを成功扱いにしません。復旧時も更新日時を切り替え、下書きの消去に失敗した場合に古い入力を再読み込みで復元しないようにします。
